...
def get_font(font_size):
    """フォントを取得し、存在しない場合は適切な代替フォントを使用"""
    possible_paths = [
        "/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf",  # DejaVu Serif（代替フォント）
        "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",  # DejaVu Sans（別の代替フォント）
    ]

    for path in possible_paths:
        if os.path.exists(path):
            return ImageFont.truetype(path, max(font_size, 20))  # 最小サイズ20を確保
...
def add_scale_bar(image, magnification):
    ...
    font_size = max(int(w * 0.05), 20)  # 最小フォントサイズを20にする
